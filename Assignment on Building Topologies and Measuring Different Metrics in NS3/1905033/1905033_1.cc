...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE("OfflineStaticScriptExample");
...
main(int argc, char* argv[])
{
    ...
    Config::SetDefault("ns3::RangePropagationLossModel::MaxRange", DoubleValue(coverage * 5));
    ...
    nodePositionList2->Add(Vector(3.0, ((nodes/2-1)*3.0)/2.0, 0.0)); // AP
    mobility.SetPositionAllocator(nodePositionList2);
    mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    mobility.Install(nodeX);
    Ptr<ListPositionAllocator> nodePositionList3 = CreateObject<ListPositionAllocator>();
    for(double i=0;i<nodes/2;i++){
        nodePositionList3->Add(Vector(26.0, i*3.0, 0.0));
    }
    mobility.SetPositionAllocator(nodePositionList3);
    mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    mobility.Install(receivers);
    Ptr<ListPositionAllocator> nodePositionList4 = CreateObject<ListPositionAllocator>();
    nodePositionList4->Add(Vector(23.0, ((nodes/2-1)*3.0)/2.0, 0.0)); // AP
    mobility.SetPositionAllocator(nodePositionList4);
    mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    ...
    case 3:
        ...
        break;
    case 4:
        NS_LOG_UNCOND(coverage << "\t" << averageThroughput << "\t" << deliveryRatio);
        break;
    }
